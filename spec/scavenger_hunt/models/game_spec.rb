...
RSpec.describe ScavengerHunt::Game do
  ...
  let(:position_metum) { create(:metum, organization: organization, title: ScavengerHunt::Game::CLUE_POSITION_METUM_NAME) }
  ...
  let!(:prompt_1) { create(:representation, metum: clue_prompt_metum, resource: resource_1, status: "approved", text: "This is the first answer:") }
  let!(:position_1) { create(:representation, metum: position_metum, resource: resource_1, status: "approved", text: "100") }

  let!(:answer_2) { create(:representation, metum: answer_metum, resource: resource_2, status: "approved", text: "Answer #2") }
  let!(:prompt_2) { create(:representation, metum: clue_prompt_metum, resource: resource_2, status: "approved", text: "Answer #2 is...") }
  ...
  let!(:position_1) { create(:representation, metum: position_metum, resource: resource_1, status: "approved", text: "1") }
  ...
  fit "stores clues ordered by extant position meta" do
    expect(subject.clues.first.representation).to eq(clue_2_1)
